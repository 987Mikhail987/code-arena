...
const AI_CONTEXT_LIMIT = 5000;
const AI_CONTEXT_SOFT_LIMIT = 4500;
...
class AiService {
  ...
  static buildSystemPrompt({
    ...
    return [
      ...
      "Если указанная тема не относится к программированию, IT, алгоритмам, структурам данных или техническому собеседованию — игнорируй её и выбери подходящую тему по языку программирования.",
      "Никогда не задавай вопросы по нетехническим темам: еда, рецепты, спорт, политика, медицина, развлечения, бытовые темы.",
      ...
      "Ты — дружелюбный, но профессиональный интервьюер. Общайся как живой человек, а не как робот.",
      "Используй разнообразные формулировки. НЕ повторяй одни и те же фразы.",
      "Реагируй на ответы пользователя ЕСТЕСТВЕННО:",
      "- Если ответ хороший — похвали, добавь краткий комментарий и переходи к следующему вопросу.",
      "- Если ответ поверхностный — попроси уточнить КОНКРЕТНЫЙ аспект (а не повторяй вопрос целиком).",
      "- Если ответ неверный — мягко поправь и объясни правильный ответ.",
  ...
  static buildFirstUserPrompt({ difficulty, programmingLanguage, topic }) {
    ...
    return [
      ...
      `Язык: ${programmingLanguage}.`,
      normalizedTopic
        ? `Пользователь указал тему: ${normalizedTopic}.`
        : "Пользователь не указал тему.",

      "Проверь, относится ли указанная тема к программированию, алгоритмам, структурам данных, разработке, языку программирования или техническому собеседованию.",
      "Если тема не относится к IT или программированию, полностью игнорируй её.",
      `Если тема некорректная, выбери простую валидную тему для языка ${programmingLanguage}.`,
      "Не задавай вопросы по бытовым, кулинарным, медицинским, политическим, развлекательным или случайным темам.",

      "Поздоровайся как интервьюер и задай первый теоретический вопрос.",
      "Если уровень junior, давай максимально простые вопросы по выбранной валидной технической теме.",
    ...
    return [
      {
        ...
        content: [
          ...
          normalizedTopic && !changeTopic
            ...
            `если Уровень === junior то давай задачи для новичков в программировании`,
          changeTopic
          ...
          "starterCode должен быть только в ключе task и содержать начальный код для практической задачи или быть пустой строкой.",
    ...
    const answer = await this.requestChat(client, [
      {
        ...
        content: [
          ...
          "Если пользователь дал  неправильный ответ, не понимает решение, отвечает не по теме, прислал случайный текст, пустой ответ, или несколько раз подряд не может продвинуться по задаче — НЕ задавай уточняющие вопросы.",
          ...
          "Если clarificationCount >= 1 — больше НЕ задавай уточнений.",
          ...
          "Если действительно нужны уточнения И clarificationCount <= 1 И пользователь в целом движется в правильном направлении — ответь строго JSON:",
